...
- A plan looks plausible on its face but hasn't been checked against edge
  cases, hard rules, or the parts of the spec it touches.
...
1. **Read the plan and its context first.** Understand exactly what is being
   proposed, and read the parts of `docs/spec.md` and `AGENTS.md` it
   touches — especially the [Hard rules](../../../AGENTS.md#hard-rules) — before
...
3. **Interview at the level of concrete failure, not vibes.** Aim for
   ...
   - **Hard-rule checks** — Does this plan hold every hard rule the spec
     requires (determinism, locked tech choices, scope), or does it quietly
     assume one away?
   - **Failure modes** — What's the first thing that breaks under bad data,
     a malformed file, or a step done out of order? How would you *know* it
     broke?
   - **Assumptions** — What is this plan silently assuming about the input,
     the environment, or the caller that might not hold?
   ...
   - **Blast radius** — What else in the codebase (or `docs/spec.md` sections)
...
- If grilling surfaces a real gap outside the current issue's scope (a spec
  ambiguity, a missing test case, an idea for later), don't fold it into
