...
1. **Read what's already there first.** Skim the request, any linked issue,
   and relevant docs (for this repo: the matching section of `docs/spec.md`,
   plus `AGENTS.md`) before asking anything — never ask a question the
   existing material already answers.
...
3. **Interview at the conceptual level, not the implementation level.** Aim
   ...
   - **Success** — What does "done and right" look like? How will you know
     it worked?
   - **Constraints** — What's fixed (spec behavior, hard rules, deadlines,
     compatibility) and what's actually negotiable?
   ...
   - **Precedent** — Is there an existing pattern in this codebase (or
     `docs/spec.md`) this should follow, or is this deliberately a departure?
