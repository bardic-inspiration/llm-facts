...
fixtures, or CI/build config. The full TDD/CI gate in [`AGENTS.md`](../AGENTS.md)
and [`CONTRIBUTING.md`](../CONTRIBUTING.md) exists to protect code
correctness; it has nothing to check on a change that contains no code.

## What qualifies

A change is **docs-only** if:

- Every changed file has a `.md` extension (`docs/spec.md`, `AGENTS.md`,
  `CLAUDE.md`, `CONTRIBUTING.md`, `README.md`, anything under `docs/`,
  `.github/ISSUE_TEMPLATE/*.md`, `.github/pull_request_template.md`, etc.).
- Nothing under `llm_facts/`, `tests/`, or `action/` changed.
- Nothing under `.github/workflows/`, `pyproject.toml`, or any lockfile
  changed.
...
- **Substantive** doc/spec changes (a spec amendment, a new or restructured
  ...
  to test. See [`issue-standards.md`](issue-standards.md).
- **Trivial, mechanical** fixes (typos, broken links, formatting, a stale
  ...
  `Closes #N`.
- If a `docs/spec.md` change is involved, it still follows
  [`spec-guidelines.md`](spec-guidelines.md) (versioning, keeping the numbered
  sections intact) regardless of which issue path you took.

## PR & commit

- Commit type is `docs` ([`commit-standards.md`](commit-standards.md)).
  Atomic commits still apply.
- **Skip:** TDD, running `ruff check . && ruff format . && pytest` locally,
...
- **Still required:** correct content, links that resolve, cross-references
  ...
  ([`documentation-standards.md`](documentation-standards.md), "Product
  docs, not process") — plus `spec-guidelines.md` versioning if
...
`ci.yml` detects a Markdown-only diff and skips the install/lint/test steps
automatically — the job still runs and reports green, it just has nothing to
build. You don't need to do anything to trigger this; it's based on the
changed file list, not a PR label or flag.
